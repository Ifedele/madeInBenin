  ...
  <script>
    $(document).ready(function(){
        $('.toggle*switch').change(function(){
            let id = $(this).data('id');
            let etat = $(this).is(':checked') ? 1 : 0;

            $.ajax({
                url: '/categorie/toggle/'+id,
                type: 'PUT',
                data: {
                    _token:'{{csrf_token() }}'
                },
                success: function(response) {
                    let message = response.etat ? "Catégorie activée" : "Catégorie désactivée";
                    alert(message);
                }
                error: function(){
                    alert ("Erreur lors du chargement d'état");
                }
            });
        });
    });
