...
<div :class="menuToggle ? 'flex' : 'hidden'" class="w-full items-center justify-between gap-4 px-5 py-4 shadow-theme-md lg:flex lg:justify-end lg:px-0 lg:shadow-none">
    <div class="flex items-center gap-4 px-5 py-4 shadow-theme-md lg:justify-end lg:px-0 lg:shadow-none">
         <!-- Notification Menu Area -->
            <!-- Dropdown Start -->
            <div class="relative" x-data="{ dropdownOpen: false, notifying: {{ auth()->user()->unreadNotifications()->count() > 0 ? 'true' : 'false' }}, isModalOpen: false, notificationMessage: '', notificationDetailsUrl: '' }" @click.outside="dropdownOpen = false; isModalOpen = false">
                <button id="notification-button" class="hover:text-dark-900 relative flex h-11 w-11 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white" @click.prevent="dropdownOpen = ! dropdownOpen; notifying = false">
                    ...
                    <span id="notification-count" class="absolute -top-1 right-0.5 z-2 inline-flex items-center justify-center px-2 py-0.5 text-xs font-bold leading-none text-red-100 bg-red-600 rounded-full"  x-show="{{ auth()->user()->unreadNotifications()->count() }} > 0" x-text="{{ auth()->user()->unreadNotifications()->count() }}"></span>
                ...
                <div x-show="dropdownOpen" class="absolute -right-[240px] mt-[17px] flex h-[480px] w-[350px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark sm:w-[361px] lg:right-0">
                    <div
                    ...
                    <ul class="custom-scrollbar flex h-auto flex-col overflow-y-auto">
                        ...
                        <li @click.prevent="isModalOpen = true; notificationMessage = '{{ $notification->data['message'] }}'; notificationDetailsUrl = '{{ $notification->data['details_url'] ?? '' }}';">
                                <a
                                    ...
                                    @if ($notification->data['details_url'])
                                        href="{{ $notification->data['details_url'] }}"
                                    @endif
                                >
                                    <span class="block">
                                        <span
                        ...
                        @empty
                            <li>
                                <p class="p-4 text-center text-gray-500 dark:text-gray-400">Aucune nouvelle notification.</p>
                            </li>
                        @endforelse
                    </ul>
                    @if ($notifications->isNotEmpty())
                        <div class="border-t border-gray-100 p-3 dark:border-gray-800">
                            <a href="{{ route('admin.dashboard') }}" class="block text-center text-theme-sm font-medium text-primary-500 hover:text-primary-600 dark:text-primary-400 dark:hover:text-primary-500">
                        ...
                        <div @click.outside="isModalOpen = false" class="relative w-full max-w-[507px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                            <div class="text-center">
                                ...
                                <div class="flex items-center justify-center w-full gap-3 mt-8">
                                    ...
                                    <a :href="notificationDetailsUrl" x-show="notificationDetailsUrl" target="_blank" type="button" class="flex justify-center px-4 py-3 text-sm font-medium text-white rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600">
                                        Voir plus
                                    </a>
                                </div>
                            </div>
        ...
        @php
            ...
            $nom = $profil->prenom ?? 'Utilisateur';
            $prenom = $profil->nom ?? '';
            $photo = $profil->photo_profile ?? null;
        @endphp
        ...
        <!-- Dropdown Start -->
        <div x-show="dropdownOpen" class="absolute right-0 mt-[17px] flex w-[260px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark">
            <div>
